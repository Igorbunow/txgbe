...
#if (defined(CONFIG_UOS_KERNEL) || defined(CONFIG_EULER_KERNEL) || \
...
/*
 * kernel below 6.2, .ptp_adjfreq is not absent, so use ptp_adjfreq
 * default, because compile failed due to .ptp_adjfine/mul_u64_u64_div_u64
 * in some old kernel version.
 */
#if (LINUX_VERSION_CODE < KERNEL_VERSION(6,2,0))
#undef NEED_MUL_U64_U64_DIV_U64
#undef NEED_DIFF_BY_SCALED_PPM
#undef HAVE_PTP_CLOCK_INFO_ADJFINE
#else /* >=6.2.0*/
#endif /* 6.2.0 */
