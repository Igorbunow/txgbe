...
function gen-flow-dissector() {
	...
	gen NEED_FLOW_MATCH if struct flow_match absent in include/net/flow_offload.h
}
...
function gen-other() {
	...
	gen HAVE_ETH_EXTENDED_HEADLEN if fun eth_get_headlen matches 'net_device' in include/linux/etherdevice.h
	...
	gen NEED_PTP_SYSTEM_TIMESTAMP if struct ptp_system_timestamp absent in include/linux/ptp_clock_kernel.h
	gen NEED_PTP_SYSTEM_PRETS if fun ptp_read_system_prets absent in include/linux/ptp_clock_kernel.h
